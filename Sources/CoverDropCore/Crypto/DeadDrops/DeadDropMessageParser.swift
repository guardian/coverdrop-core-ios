...
enum DeadDropMessageParser {
    ...
    static func parseMessage(messageBytes: [UInt8], journalistData: JournalistData, deadDropId: Int, dateReceived: Date) -> Message? {
        ...
        if firstByte == DeadDropMessageParser.typeFlagMessage {
            return parseTextMessage(messageBytes: remainingMessageBytes, journalistData: journalistData, deadDropId: deadDropId, dateReceived: dateReceived)
        } else if firstByte == DeadDropMessageParser.typeFlagHandover {
            return parseHandoverMessage(messageBytes: remainingMessageBytes, journalistData: journalistData, deadDropId: deadDropId, dateReceived: dateReceived)
        } else {
            return nil
        }
    }
