...
public enum UserToCoverNodeMessage {
    ...
    public static func createMessage(message: String, recipientPublicKey: JournalistMessagingPublicKey, verifiedPublicKeys: VerifiedPublicKeys, userPublicKey: PublicEncryptionKey<User>, tag: RecipientTag) throws -> MultiAnonymousBox<UserToCoverNodeMessageData> {
        let innerMessage: AnonymousBox<UserToJournalistMessageData> = try UserToJournalistMessage.encryptRealMessageFromUserToJournalist(recipentPublicKeyData: recipientPublicKey, userPublicKey: userPublicKey, message: message)
        let keys = verifiedPublicKeys.mostRecentCoverNodeMessagingKeysFromAllHierarchies()
        let message = try UserToCoverNodeMessage.encryptRealMessageFromUserToCovernode(verifiedPublicKeys: keys, userToJournalistMessage: innerMessage, recipientIdenitfierTag: tag)
        return message
    }
    ...
    static func encryptRealMessageFromUserToCovernode(verifiedPublicKeys: [CoverNodeIdentity: CoverNodeMessagingPublicKey], userToJournalistMessage: AnonymousBox<UserToJournalistMessageData>, recipientIdenitfierTag: RecipientTag) throws -> MultiAnonymousBox<UserToCoverNodeMessageData> {
