...
final class VerifyTestVectors: XCTestCase {
    func testAnonymousBoxTest() throws {
        let recipientPk = try PublicEncryptionKey<JournalistMessaging>(key: XCTUnwrap(FileHelper()
        ...
        let recipientSk = try SecretEncryptionKey<JournalistMessaging>(key: Box
            .SecretKey(XCTUnwrap(FileHelper().bytesFromFile(filePath: "vectors/anonymous_box/02_recipient_sk"))))

        let message = try XCTUnwrap(try FileHelper().bytesFromFile(filePath: "vectors/anonymous_box/03_message"))

        let anonymousBox: AnonymousBox<[UInt8]> = try AnonymousBox(pkTagAndCiphertext: XCTUnwrap(FileHelper()
    ...
    func testTwoPartyBoxTest() throws {
        let senderPk = try PublicEncryptionKey<JournalistMessaging>(key: XCTUnwrap(FileHelper()
                .bytesFromFile(filePath: "vectors/two_party_box/01_sender_pk")))

        let recipientSk = try SecretEncryptionKey<User>(key: Box
            .SecretKey(XCTUnwrap(FileHelper().bytesFromFile(filePath: "vectors/two_party_box/04_recipient_sk"))))

        let message = try XCTUnwrap(try FileHelper().bytesFromFile(filePath: "vectors/two_party_box/05_message"))

        let twoPartyBox: TwoPartyBox<[UInt8]> = try TwoPartyBox(tagCiphertextAndNonce: XCTUnwrap(FileHelper()
    ...
    func testMultiAnonymousBoxTest() throws {
        let recipient1Pk = try PublicEncryptionKey<CoverNodeMessaging>(key: XCTUnwrap(FileHelper()
        ...
        let recipient1Sk = try SecretEncryptionKey<CoverNodeMessaging>(key: Box
            .SecretKey(XCTUnwrap(FileHelper()
                    .bytesFromFile(filePath: "vectors/multi_anonymous_box/02_recipient_1_sk"))))

        let recipient2Pk = try PublicEncryptionKey<CoverNodeMessaging>(key: XCTUnwrap(FileHelper()
        ...
        let recipient2Sk = try SecretEncryptionKey<CoverNodeMessaging>(key: Box
            .SecretKey(XCTUnwrap(FileHelper()
                    .bytesFromFile(filePath: "vectors/multi_anonymous_box/04_recipient_2_sk"))))
        ...
        let message = try XCTUnwrap(try FileHelper().bytesFromFile(filePath: "vectors/multi_anonymous_box/05_message"))

        let multiAnonymousBox: MultiAnonymousBox<[UInt8]> = try MultiAnonymousBox(bytes: XCTUnwrap(FileHelper()
